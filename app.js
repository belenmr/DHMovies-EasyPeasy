...
http.createServer((req, res) => {
	...
	switch (req.url) {
		// Home
		case '/':
			// res.write para escribir en pantalla
		...
		case '/en-cartelera':
			...
			break;
		case '/mas-votadas':
			let mostVotedMovies = movies.filter(movie => movie.vote_average >= 7);
			let accumulator = 0;
			...
			res.write('Más Votadas \n\n');
			res.write(`Total de películas: ${mostVotedMovies.length} \n\n`);
			res.write('Rating promedio: ' + (accumulator/mostVotedMovies.length).toFixed(2) + '\n\n');
			mostVotedMovies.forEach(element => {
				res.write(`${element.title} \n`);
		...
		case '/contacto':
			...
			res.end('​¿Tenés algo para contarnos? Nos encanta escuchar a nuestros clientes. Si deseas contactarnos podés escribirnos al siguiente email: dhmovies@digitalhouse.com o en las redes sociales. Envianos tu consulta, sugerencia o reclamo y será respondido a la brevedad posible. Recordá que también podes consultar la sección de Preguntas Frecuentes para obtener respuestas inmediatas a los problemas más comunes. ');
			break;
